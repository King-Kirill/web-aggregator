...
export default function Category({setOpenModal, setModalType, setHandlerType = null, setStatusCode = null, setToastMessage = null, setCategoryId = null, setCategoryName = null, setCategoryImage = null, setCategoryAdress = null, setCategoryAmount=null, setVisibleAuth=null, setLoading=null, setOpenMenu=null}) {
  ...
  const handleClickaddCategoryGroup = async () => {
  setOpenModal(true);
  setModalType("addCategoryGroup");
  if(setHandlerType)
    {
      setHandlerType("addCategoryGroup");
    }
  };
  ...
  const handleUpdateCategoryGroup = async (id, name, adress, image, amount) => {
    setCategoryId(id);
    setCategoryName(name);
    setCategoryAdress(adress);
    setCategoryImage(image);
    setCategoryAmount(amount);
    setOpenModal(true);
    setModalType("updateCategoryGroup");
    if(setHandlerType)
    {
      setHandlerType("updateCategoryGroup");
    }
  };

  const  handleDeleteCategoryGroup = async (id) => {
      try {
        setLoading(true);
        const res = await fetch(`${BASE_URL}/delete-category/${id}`, {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className="outer-container">
        <div className="grid-container">
          {items.map((item) => (
            <div key={item.id} className={`grid-container-item ${isAdmin ? "admin" : ""}`}>
              {isAdmin && (
                <button className="grid-container-on-delete" onClick={() => {
                      if (window.confirm("Вы уверены, что хотите безвозвратно удалить элемент?")) {
                      handleDeleteCategoryGroup(item.id);}}}>
              ...
              {checkPath(item.api_adress) ? (
                <Link className={`grid-container-item-main-btn ${isAdmin? "admin":""}`} to={item.api_adress} onClick={() => handleNavigate()}>
                ...
                <a href={item.api_adress} className={`grid-container-item-main-btn ${isAdmin ? "admin" : ""}`} target="_blank" rel="noopener noreferrer" onClick={() => handleNavigate()}>
                  <div className="grid-item">
                <div className="image-wrapper">
                  <img loading="lazy" src={item.image_src} alt={item.name} />
                  <div className="category-overlay">
            ...
            {isAdmin && (
              <button className="grid-container-on-change" onClick={() => handleUpdateCategoryGroup(item.id, item.name, item.api_adress, item.image_src, item.amount)}>
                            <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
          ...
          {isAdmin && (
            <button className="addShopCategory" onClick={handleClickaddCategoryGroup}>
                    <svg version="1.0" xmlns="http://www.w3.org/2000/svg"

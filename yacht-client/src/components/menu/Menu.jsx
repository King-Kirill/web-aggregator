...
export default function Menu({data, onDelete, onUpdate, onCreate, onDeleteFromPage, groupId, compId})
{
  ...
  const sortedAdData = sortByOrderId(data);
    ...
    return (
    <>
       <div className={`menu-container-${gridCols}`}>
        {isAdmin &&(
                <button className="menu-container-on-delete-from-page" onClick={() => onDeleteFromPage(compId)}>
        ...
        {isAdmin && (
                    <button className="menu-container-item-btn" onClick={() => onCreate(groupId)}>
                        <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
        ...
        {data.map((item, id) => (
            <div key={id} className="menu-container-item-cover">
                <div className={`menu-container-item ${isAdmin ? "admin" : ""}`} key={id}
                style={{
                background: `
                ...
                {isAdmin && (
                    <button className="menu-container-on-change" onClick={() => onUpdate(item.id, item.pdf_ref, item.image_src, item.title, item.text, item.order_id, item.group_id)}>
                        <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                ...
                {isAdmin && (
                    <button className="menu-container-on-delete" onClick={() => {
                ...
                <div className="menu-container-item-content">
                    <span className="menu-container-item-content-name">
                        {item.title}
                    </span>
                    <p>
                        {item.text}
                    </p>
                    <div className="menu-container-item-content-btn-wrapper">
                        <button onClick={() => window.open(item.pdf_ref, "_blank")}>
                            подробнее
                        </button>
                    </div>
                </div>
    ...
    <div className='menu-container-swiper'>
        <Swiper
        spaceBetween={50}
        loop={true}
        autoplay={{
    ...
    >
        {data.map((item, id) => (
            <SwiperSlide key={id} className="menu-container-item-cover">
                <div className={`menu-container-item ${isAdmin ? "admin" : ""}`} key={id}
                style={{
                background: `
                    linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.5)),

...
export default function Footer()
{
    ...
    const [isAdmin, setIsAdmin] = useState(window.location.port === "5174" && window.innerWidth > 1200);
  ...
  useEffect(() => {
    ...
    const handleDeleteFooterBtn = (id) => {
    setFooterBtns((prev) => prev.filter((btn) => btn.id !== id));
    };

    const onDeleteFooter = async (id) => {
        try {
      setLoading(true);
      const res = await fetch(`${BASE_URL}/footer-delete/${id}`, {
        method: "DELETE",
        headers: {
      ...
      if (res.status === 200) {
              ...
              setToastMessage("Элемент удален успешно!");
              handleDeleteFooterBtn(id);
    ...
    const createFooterItem = () => {
        setHandlerType("handleAddFooterBtn");
        setTemplateType("addFooter");
        setOpenModal(true);
    }

    const updateFooterItem = (item) => {
        setItemId(item.id);
        setItemName(item.name);
        setItemAdress(item.api_adress);
        setHandlerType("handleUpdateFooterBtn");
        setTemplateType("updateFooter");
        setOpenModal(true);
    }
    ...
    try {
      setLoadingPage(true);
      const res = await fetch(`${BASE_URL}/get-footer`);

      if (res.status === 200) {
        const data = await res.json();
        setFooterBtns(data.content);
      }
...
const handlers = {
    handleAddFooterBtn: (item) => {
    setFooterBtns((prev) => [...prev, item]);
    },

    handleUpdateFooterBtn: (id, item) => {
    setFooterBtns((prev) =>
      prev.map((btn) =>
        btn.id === id ? { ...btn, ...item } : btn
      )
    );
  }
}
         ...
         <section className ="footer">
            {loadingPage ? (
                ...
                <div className="footer-buttons">
                  ...
                  <button onClick={() => {navigate("/rent-policy")}}>Условия заказа</button>
                  <button onClick={() => {navigate("/contacts")}}>Контакты и реквизиты ИП</button>
                    {footerBtns.map((item) => (
                        <>
                      {checkPath(item.api_adress) ? (
                        <Link className="footer-btn-link" key={item.id} to={item.api_adress}>
                            {item.name}
                            {isAdmin &&(
                                                                      <button className="footer-on-delete-btn" onClick={() => {
                                                                      if (window.confirm("Вы уверены, что хотите безвозвратно удалить элемент?")) {onDeleteFooter(item.id)}}}>
                                                                    ...
                                                                    {isAdmin &&(
                                                                      <button className="footer-on-update-btn" onClick={() => updateFooterItem(item)}>
                                                                      <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <a className="footer-btn-link" href={item.api_adress} key={item.id} target="_blank" rel="noopener noreferrer">
                            {item.name}
                            {isAdmin &&(
                                                                      <button className="footer-on-delete-btn" onClick={() => {
                                                                      if (window.confirm("Вы уверены, что хотите безвозвратно удалить элемент?")) {onDeleteFooter(item.id)}}}>
                    ...
                    {isAdmin && (
                        <button className="footer-add-btn" onClick={() => {createFooterItem()}}>
                            <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                ...
                >
                  ...
                  <meta itemProp="telephone" content="+79643333636" />
                  <meta itemProp="email" content="noreplyvipboat@yandex.ru" />
                  <Link to="/" itemProp="url">
                    <img
                      loading="lazy"
                      className="footer-logo"
                      src="/images/logo.png"
                      alt="Vip Boat — аренда яхт в Санкт-Петербурге"
                      itemProp="logo"
                    />
                  </Link>
                  <span className="footer-right" itemProp="description">
                    © VipBoat, 2026. Все права защищены. Аренда яхт, катеров и теплоходов в Санкт-Петербурге. Копирование материалов запрещено. Сайт не является публичной офертой.
                  </span>
                  <span className="footer-IM">
                    <span itemProp="legalName">ИП Кузьмык Павел Иосифович</span>
                    {" "}(ИНН <span itemProp="taxID">780159592304</span>, ОГРНИП{" "}
